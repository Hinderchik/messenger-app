...
export default async function handler(req, res) {
  ...
  try {
    ...
    const user = await client.query('SELECT id, username FROM users WHERE email = $1', [email]);
    ...
    res.status(200).json({ message: 'Ссылка для подтверждения отправлена на почту' });
  } catch (error) {
    console.error(error);
